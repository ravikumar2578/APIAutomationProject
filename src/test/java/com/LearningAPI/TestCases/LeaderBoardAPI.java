...
public class LeaderBoardAPI extends BaseTest {
	...
	public void init() {
		...
		test = rep.startTest("LeaderBoardAPI");
	...
	@Test(dataProvider = "getData", priority = 0, testName = "Verify LeaderBoard API")

	public void LeaderBoardAPI_Test(Hashtable<String, String> data) throws Exception {

		String Checksum_Data = data.get("action") + ":" + data.get("user_id") + ":" + data.get("board_id") + ":"
				+ data.get("class_id") + ":" + data.get("service_id") + ":" + Constants.APIKey + ":" + Constants.Salt;

		test.log(LogStatus.INFO, "LeaderBoardAPI Test started");
		if (!DataUtil.isTestRunnable(xls, "LeaderBoardAPI") || data.get("Runmode").equals("N")) {
		...
		String Resource = "api/v1.0/leaderboard";

		String Querystring = "?" + "action=" + data.get("action") + "&user_id=" + data.get("user_id") + "&board_id="
				+ data.get("board_id") + "&class_id=" + data.get("class_id") + "&service_id=" + data.get("service_id")
	...
	public Object[][] getData() {
		...
		Object[][] data = DataUtil.getData(xls, "LeaderBoardAPI");
		return data;
	}
