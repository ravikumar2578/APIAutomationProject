...
public class UserRegistration_ForgotPasswordAPI extends BaseTest {
	...
	public void init() {
		...
		test = rep.startTest("UserRegistration_ForgotPasswordAPI");
	...
	@Test(dataProvider = "getData", priority = 0, testName = "Verify Forgot-Password API")

	public void UserRegistration_ForgotPasswordAPI_Test(Hashtable<String, String> data) throws Exception {
		test.log(LogStatus.INFO, "UserRegistration_ForgotPasswordAPI Test started");
		if (!DataUtil.isTestRunnable(xls, "UserRegistration_ForgotPasswordAPI") || data.get("Runmode").equals("N")) {

			test.log(LogStatus.SKIP, "Skipping the test");

		}

		String PostRequest_checkusm = data.get("action") + ":" + data.get("value") + ":" + data.get("type") + ":"
				+ data.get("otp_code") + ":" + data.get("newpwd") + ":" + data.get("user_id") + ":" + Constants.APIKey
				+ ":" + Constants.Salt;
		...
		inner_requestParams.put("value", data.get("value"));
		inner_requestParams.put("type", data.get("type"));
		inner_requestParams.put("otp_code", data.get("otp_code"));
		inner_requestParams.put("newpwd", data.get("newpwd"));
		inner_requestParams.put("user_id", data.get("user_id"));
		requestParams.put("forgot_details", inner_requestParams);
	...
	public Object[][] getData() {
		...
		Object[][] data = DataUtil.getData(xls, "UserRegistration_ForgotPasswordAPI");
		return data;
	}
